...
// search/replace all on this page ------>  xHx  <-------- to set purchased server name to unique choice.
	var hackFiles = ['wLoop.script', 'gLoop.script', 'hLoop.script', 'ws.script', 'gs.script', 'hs.script','healthMonitor.js'];
	//...................[0]............[1]............[2]...............[3]..........[4]..........[5].............
	...
	ns.exec('healthMonitor.js', 'foodnstuff')
...
//			}
			...
			while (t < 1) {
				if (ns.getServerRequiredHackingLevel(target) <= ns.getHackingLevel()) {
					...
					var gLthreads = Math.floor((ns.getServerMaxRam(servers[s]) / 3) / 1.75);
					if (gLthreads <= 1) { ++gLthreads; }

					var hLthreads = Math.floor((ns.getServerMaxRam(servers[s]) / 3) / 1.90);
					if (hLthreads <= 1) { ++hLthreads; }

					var hsthreads = Math.floor((ns.getServerMaxRam(servers[s]) / 3) / 1.7);
					if (hsthreads <= 1) { ++hsthreads; }

					var homegLthreads = Math.floor(((ns.getServerMaxRam('home') / 3) / 1.75) );
					if (homegLthreads <= 1) { ++homegLthreads; }

					var homehLthreads = Math.floor((ns.getServerMaxRam('home') / 3) / 1.90);
					if (homehLthreads <= 1) { ++homehLthreads; }

					var homehsthreads = Math.floor((ns.getServerMaxRam('home') / 3) / 1.7);
					...
					ns.exec('gLoop.script', (servers[s]), gLthreads, target);
					ns.exec('wLoop.script', (servers[s]), gLthreads, target);
					if (ns.getServerMoneyAvailable(target) == (ns.getServerMaxMoney(target) )) 
					{ ns.exec('hLoop.script', 'home', homehLthreads, target); }
					if (ns.getServerMoneyAvailable(target) < (ns.getServerMaxMoney(target))*.99) {
						ns.exec('gs.script', 'home', homegLthreads, target);
					}

					var targetGWsThreads = Math.floor(((ns.getServerMaxRam(target) - ns.getServerUsedRam(target)) / 1.75) / target.length);
					if (targetGWsThreads <= 1) { ++targetGWsThreads; }
					var targetHsThreads = Math.floor(((ns.getServerMaxRam(target) - ns.getServerUsedRam(target)) / 1.7) / target.length);
					if (targetHsThreads <= 1) { ++targetHsThreads; }


					if (ns.getServerSecurityLevel(target) > (ns.getServerMinSecurityLevel(target) + .8192)) {
						ns.exec('ws.script', servers[s], gLthreads, target);
						ns.exec('ws.script', target, targetGWsThreads, target);
					}

					if (ns.getServerMoneyAvailable(target) < (ns.getServerMaxMoney(target))*.99) {
						ns.exec('gs.script', servers[s], (gLthreads/1), target);
					}


					if (ns.getServerMoneyAvailable(target) == (ns.getServerMaxMoney(target))) {
						ns.exec('hs.script', servers[s], (hsthreads/1), target);
					}
