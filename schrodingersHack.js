...
// search/replace all on this page ------>  xHx  <-------- to set purchased server name to unique choice.
	...
	await ns.write('purchaseServer.js', "/** @param {NS} ns **/export async function main(ns) {var ram = 32;var i = 0;while (i < 25) {var hostname = ns.purchaseServer('xHx-' + i, ram);while (ns.getServerMoneyAvailable('home') < ns.getPurchasedServerCost(ram)*25) {await ns.hack('n00dles');}ns.tprint('buying ', hostname, ' @ ', ram, 'GB for $', ns.getPurchasedServerCost(ram));await ns.scp('gLoop.script', hostname);await ns.scp('wLoop.script', hostname);ns.exec('gLoop.script', hostname, ((ram / 1.75)/4), 'n00dles');ns.exec('wLoop.script', hostname, ((ram / 1.75)/4), 'n00dles');ns.exec('gLoop.script', hostname, ((ram / 1.75)/4), 'joesguns');ns.exec('wLoop.script', hostname, ((ram / 1.75)/4), 'joesguns');++i;await ns.hack('n00dles');}ns.spawn('purchaseServerUpgrade.js')}", 'w');
		...
		while (s < servers.length) {
			...
			while (t < target.length) {
				if (ns.getServerRequiredHackingLevel(target[t]) <= ns.getHackingLevel()) {
					...
					if (ns.getServerMoneyAvailable(target[t]) < (ns.getServerMaxMoney(target[t]))*.99) {
						ns.exec('gs.script', servers[s], (gLthreads/target.length), target[t]);
					}


					if (ns.getServerMoneyAvailable(target[t]) == (ns.getServerMaxMoney(target[t]))) {
						ns.exec('hs.script', servers[s], (hsthreads/target.length), target[t]);
					}
