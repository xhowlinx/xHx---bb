/** @param {NS} ns **/export async function main(ns) {
	...
	await ns.write(files[11],"/** @param {NS} ns **/export async function main(ns) {ns.tail();var i = 0;var ram = ns.getServerMaxRam('xHx-0');while(i<ns.getPurchasedServerLimit()){ns.killall('xHx-'+i);ns.deleteServer('xHx-'+i);ns.purchaseServer('xHx-'+i,ram*2);await ns.scp('n00dleshack.js',('xHx-'+i));ns.exec('n00dleshack.js',('xHx-'+i),((ram*2)/2.4));++i;}}","w");
	...
	await ns.write(files[15],"/** @param {NS} ns **/export async function main(ns) {ns.tail();ns.kill('n00dleshack.js','home');var threads = (Math.floor(ns.getServerMaxRam('home')-12)/2.4);ns.run('n00dleshack.js',threads);}","w");
...
await ns.sleep(5000);
ns.run('pushhome.js');
if(ns.fileExists('SQLinject.exe'))await ns.sleep(1000);
