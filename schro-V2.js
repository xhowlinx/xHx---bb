...
//			}
			...
			while (t < 1) {
				if (ns.getServerRequiredHackingLevel(target) <= ns.getHackingLevel()) {
					...
					var gLthreads = Math.floor((ns.getServerMaxRam(servers[s]) / 1) / 1.75);
					if (gLthreads <= 1) { ++gLthreads; }

					var hLthreads = Math.floor((ns.getServerMaxRam(servers[s]) / 1) / 1.90);
					if (hLthreads <= 1) { ++hLthreads; }

					var hsthreads = Math.floor((ns.getServerMaxRam(servers[s]) / 1) / 1.7);
					if (hsthreads <= 1) { ++hsthreads; }

					var homegLthreads = Math.floor(((ns.getServerMaxRam('home') / 1) / 1.75) );
					if (homegLthreads <= 1) { ++homegLthreads; }

					var homehLthreads = Math.floor((ns.getServerMaxRam('home') / 1) / 1.90);
					if (homehLthreads <= 1) { ++homehLthreads; }

					var homehsthreads = Math.floor((ns.getServerMaxRam('home') / 1) / 1.7);
					...
					ns.exec('gLoop.script', (servers[s]), 1, target);
					ns.exec('wLoop.script', (servers[s]), 1, target);
					...
					if (ns.getServerMoneyAvailable(target) < (ns.getServerMaxMoney(target))*.99) {
						ns.exec('gs.script', servers[s], (gLthreads/target.length), target);
					}


					if (ns.getServerMoneyAvailable(target) == (ns.getServerMaxMoney(target))) {
						ns.exec('hs.script', servers[s], (hsthreads/target.length), target);
					}
